...
state.completed = 0;
    ...
    Kal.Q = diag( [ (0.01)^2 ,(0.01)^2 , (1*pi/180)^2]);
    % Assume we introduce uncertainty of 0.2m/s for v and 1deg/s on omega.
    Kal.Pu = diag([0.2^2, pi/180]);
    ...
    for i = 0:10:2228850
        ...
        if (numel(scanTime)) 
            ...
            elseif num > 0
                pole_update(x, y, theta, posScan, scanTime, num, centres, polePos);
            end
...
function pole_update(x, y , theta, posScan, scanTime, num, centres, polePos)
    global DEFS;
    global plt;
    ...
    for pole=1:num
        ...
        if numel(closestPole) > 0
            ...
            if (poleAlloc(closestPole, 3) == 0 || poleAlloc(closestPole, 3) > min(distances))
                ...
                poleAlloc(closestPole,1:2) = predictedPoles(pole, :);
            end
...
function carFrame = getCarFrame(x, y, theta)
    carFrame = [0, 0; 0, 0.12 ; 0.46, 0; 0, -0.12; 0, 0];
